...
double const *genann_run(genann const *ann, double const *inputs) {
    ...
    int lenY, lenX, lda, alpha, beta, incX, incY, N;
    //##########################################################################
    //##########################################################################
    //##########################################################################
    ...
    //##########################################################################
    //##########################################################################
    // Call signature cblas:
    // /*         row_order      transform     lenY lenX alpha  a  lda  X  incX  beta  Y, incY */
    // cblas_dgemv(CblasColMajor, CblasNoTrans, 4,   5,   1,     a,   5, x, 1,    0,    y, 1);
  ...
  //  cblas_dgemv(CblasColMajor, CblasNoTrans, lenY, lenX, 1, w, lda, i, incX, beta, o, incY);
  //   //*o = *w * -1.0
  //   // increment W, i o accordingly to size
  //   w += lenX * lenY;
  //   i += lenY;
  //   o += lenY;
  //
  ...
  //       // W*i = o
  //       cblas_dgemv(CblasColMajor, CblasNoTrans, lenY, lenX, alpha, w, lda, i, incX, beta, o, incY);
  //       // Increment W, i , o correctly to fit next layer
  //       w += lenX * lenY;
  //       i += lenY;
  //       o += lenY;
...
////////////////////////////////////////////////////////////////////////////
    ...
    for (h = 0; h < ann->hidden_layers; ++h) { // ++h?????
        for (j = 0; j < ann->hidden; ++j) {
            double sum = *w++ * -1.0;
            for (k = 0; k < (h == 0 ? ann->inputs : ann->hidden); ++k) { // h==0?????
                sum += *w++ * i[k];
            }
            *o++ = act(sum);
        }
...
/// ????? HOW TO USE ACTIVATOIN FUNCTION?

 // OUTPUT LAYER
 // lenX = (ann->hidden_layers ? ann->hidden : ann->inputs);
 // lenY = ann->outputs;
 // lda = lenX;
...
///////////////////////////////////////////////////////////7777
    ...
    /* Train the outputs. */
    {
      ...
      // ax + y = y.... how to get result into w without using w??????
      // alpha = -1.0 * learning_rate;
      // incX = 1;
      // N = ann->outputs;
      // cblas_daxpy(N, alpha, d, incX, w, N);
      // w++;
      // for (k = 1; k < (ann->hidden_layers ? ann->hidden : ann->inputs) + 1; ++k) {
      //   alpha = learning_rate * i[k-1];
      //   cblas_daxpy(N, alpha, d, incX, w, N)
      //   w++;
      // }
      // w += N; // to increment pointer correctly
...
///////////////////////////////////////////////////////////////////
    ...
    for (h = ann->hidden_layers - 1; h >= 0; --h) {
        ...
        // int N = (h == 0 ? ann->inputs : ann->hidden) + 1; // Difference in length row wise weight
        // cblas_daxpy(N, -learning_rate, d, 1, w, N)
        // for (k = 1; k < (h == 0 ? ann->inputs : ann->hidden) + 1; ++k) {
        //   cblas_daxpy(N, learning_rate * i[k-1], d, 1, w, N)
        //   w++;
        // }
////////////////////////////////////////////////////////////////////////
